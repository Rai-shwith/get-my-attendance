...
const handleRegistration = (registerID, name, usn, req) => {
    logger.debug(`Entering handleRegistration : Mac Id: [${registerID}] Name[${name}] USN[${usn}]`);
    ...
    const alreadyRegisteredById = getStudentById(registerID)
    if (alreadyRegisteredById) {
        errorMessage = alreadyRegisteredById.name + "  is already registered";
    ...
    if (!alreadyRegistered) {
        ...
        // If the student is not registered then add his macAddress
        addStudent(registerID, { name, usn });
        currentRegistration.addStudent(registerID, { name, usn });
...
exports.registerStudent = (req, res) => {
    ...
    if (!macAddress) {
        logger.error('MAC address is not provided');
        sendMessage("notification", JSON.stringify({ type: 'error', message: `Unable to get the Mac address of one student` }));
        return res.status(400).json({ message: "'MAC address is not provided" });
    }
    ...
    try {
        handleRegistration(macAddress, name, usn, req)
